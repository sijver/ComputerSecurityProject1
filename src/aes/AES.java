...
/**
 * Class that implements 4-rounds AES encrypting
 */
public class AES {
    ...
    public static byte[][] getRoundKey(int round) throws NullPointerException {
        byte[][] roundKey = new byte[4][4];
        for (int i = 0; i < 4; i++) {
            roundKey[i] = w[round * 4 + i];
        }
        return roundKey;
    }
    ...
    public static byte invSubByte(byte state) {
        return (byte) (invSbox[(state & 0x000000ff)] & 0xff);
    }
